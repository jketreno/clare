---
name: mcp-server
description: "Scaffold an MCP server exposing CLARE verify and autonomy-check tools"
mode: agent
---
...
> exposes CLARE's enforcement primitives — `verify-ci.sh` and `autonomy.yml` — as typed
> tool calls. Any MCP-compatible agent or orchestrator can then call CLARE tools without
> needing bash access or CLARE-specific prompt engineering.
>
> **When to use:** When running multi-agent pipelines, headless agents, or any workflow
> where you need CLARE enforcement available as a structured tool rather than a bash script.
>
...
**For Python** — create `mcp/clare_server.py`:
...
VERIFY_SCRIPT = PROJECT_ROOT / "clare" / "verify-ci.sh"
...
def load_autonomy():
  if not AUTONOMY_FILE.exists():
    return {"error": "missing_autonomy", "message": "clare/autonomy.yml not found"}
  with open(AUTONOMY_FILE, encoding="utf-8") as f:
    return yaml.safe_load(f) or {}

@mcp.tool()
def clare_verify() -> str:
  if not VERIFY_SCRIPT.exists():
    return json.dumps({
      "status": "error",
      "error": "missing_verify_script",
      "summary": "clare/verify-ci.sh not found"
    })
...
@mcp.tool()
def clare_check_autonomy(path: str) -> str:
...
@mcp.tool()
def clare_list_humans_only() -> str:
  autonomy = load_autonomy()
  if "error" in autonomy:
    return json.dumps({"status": "error", **autonomy})
...
```json
{
  "name": "clare-mcp-server",
  "version": "1.0.0",
  "type": "module",
  "description": "CLARE MCP server — exposes verify-ci.sh and autonomy.yml as MCP tools",
  "main": "clare-server.js",
  "scripts": {
    "start": "node clare-server.js"
  },
  "dependencies": {
...
If the user wants to verify the MCP server itself starts cleanly, add to `clare/verify-local.sh`:

```bash
if [[ -f "$PROJECT_ROOT/mcp/clare-server.js" ]]; then
  run_check "CLARE MCP server syntax" "node --check $PROJECT_ROOT/mcp/clare-server.js 2>&1"
fi
```
...
```
CLARE MCP server created at mcp/. To register with Claude Code:
...
2. Add to .claude/settings.json (create if missing):
   {
     "mcpServers": {
       "clare": {
         ...
         "args": ["./mcp/clare-server.js"]   // or ["./mcp/clare_server.py"]
       }
     }
   }
...
4. Restart Claude Code / Cursor. Tools will be available as:
   - mcp__clare__clare_verify
   - mcp__clare__clare_check_autonomy
   - mcp__clare__clare_list_humans_only

See clare/docs/agentic.md for multi-agent usage patterns.
```
...
```bash
# Node.js:
cd mcp && npm install && echo '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}' | node clare-server.js

# Python:
pip install -r mcp/requirements.txt && echo '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}' | python mcp/clare_server.py
```

Expected output: a JSON response listing the three CLARE tools.

Passing tool-call validation example:

```bash
echo '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"clare_verify","arguments":{}}}' | node mcp/clare-server.js
```
