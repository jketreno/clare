...
Fill in the actual values by reading the files. Show this block **once per session**, not on every message. If `clare/autonomy.yml` is missing, warn the user to run `./scripts/clare-installer.sh --target .`.
...
Scope note for this repository: `humans-only` for `clare/verify-ci.sh` applies only to the repository root path. Files under `install/` (including `install/**/verify-ci.sh`) follow the `install` boundary and are editable.
...
- Skill files in `install/clare/templates/skills/` define how to regenerate each type
...
- `/project:release` — Publish a release and sync README "CLARE in 60 Seconds" to latest artifacts
...
| `clare/verify-ci.sh` | Root CI enforcement script — run before completing work (CLARE-owned) |
| `clare/verify-local.sh` | Project-specific checks — add your checks here |
| `scripts/clare-installer.sh` | One-time install/update/setup entrypoint |
| `install/` | Everything that gets installed into target projects |
| `install/clare/templates/` | Generic templates (architecture tests, skills, linting, CI, git hooks) |
| `install/clare/examples/` | Domain-specific examples (architecture tests, skills) |
| `install/root/CLAUDE.md` | Installed CLAUDE.md template for target projects |
...
# Setup
./scripts/clare-installer.sh --target .   # Interactive install/update/setup
