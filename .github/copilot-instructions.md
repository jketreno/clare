...
Fill in the bracketed values from the actual files. Show this block **once per conversation**, not on every message. If `clare/autonomy.yml` is missing, tell the user to run `./scripts/clare-installer.sh`.
...
- See `install/clare/templates/skills/` for regeneration skill templates
...
```
clare/verify-ci.sh                — Run this. Always. (CLARE-owned)
...
scripts/clare-installer.sh        — Install/update CLARE into target projects
scripts/release.sh                — Build and publish releases
install/                          — Everything installed into target projects
install/clare/templates/           — Generic templates (architecture tests, skills, linting, CI)
install/clare/examples/            — Domain-specific examples (architecture tests, skills)
